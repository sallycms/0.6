...
(function(c){pageloaded=false;imageExtensions=["png","gif","jpg","jpeg","bmp"];changeImage=function(a,b){c("#"+a).attr("src",b)};makeWinObj=function(a,b,e,g,f,h,i){i=i=="toolbar"?"scrollbars=yes,toolbar=yes":i=="empty"?"scrollbars=no,toolbar=no":"scrollbars=yes,toolbar=no"+i;this.name=a;this.url=b;this.obj=window.open(b,a,"width="+f+",height="+h+","+i);this.obj.moveTo(e,g);this.obj.focus()};closeAll=function(){for(var a in winObj)winObj[a].obj.close()};newWindow=function(a,b,e,g,f){if(e==0)e=550;
if(g==0)g=400;extra=f=="scrollbars"?"toolbar":f=="empty"?"empty":f;if(f=="nav"){posx=parseInt(screen.width/2)-390;posy=parseInt(screen.height/2)-314;e=320;g=580}else if(f=="content"){posx=parseInt(screen.width/2)-60;posy=parseInt(screen.height/2)-314;e=470;g=580}else{posx=parseInt((screen.width-e)/2);posy=parseInt((screen.height-g)/2)-24}winObj.push(new makeWinObj(a,b,posx,posy,e,g,extra))};winObj=[];newPoolWindow=function(a){newWindow("rexmediapopup",a,760,600,",status=yes,resizable=yes")};newLinkMapWindow=
...
a.val();else if(typeof b=="undefined")b="";newLinkMapWindow("index.php?page=linkmap&opener_input_field="+e+b)};deleteREXLinklist=function(a){deleteREX(a,"REX_LINKLIST_","REX_LINKLIST_SELECT_")};moveREXLinklist=function(a,b){moveREX(a,"REX_LINKLIST_","REX_LINKLIST_SELECT_",b)};writeREXLinklist=function(a){writeREX(a,"REX_LINKLIST_","REX_LINKLIST_SELECT_")};deleteREX=function(a,b,e){var g=c("#"+b+a+" option"),f=g.length,h=g.filter(":selected").index();if(h!=-1){g[h]=null;f--;if(h==0){if(f>0)g[0].selected=
"selected"}else if(f>h)g[h].selected="selected";else g[h-1].selected="selected";writeREX(a,b,e)}};moveREX=function(a,b,e,g){for(var f=c("#"+e+a+" option"),h=[],i=[],d=0;d<f.length;++d){i[d]=false;h[d]={value:f[d].value,title:f[d].text}}var k=0,j=[];j[-1]=true;j[f.length]=true;if(g=="top")for(d=0;d<f.length;++d)if(f[d].selected){h=moveItem(h,d,k);i[k]=true;k++}if(g=="up")for(d=0;d<f.length;++d){j[d]=false;if(f[d].selected){to=d-1;if(j[to])to=d;h=moveItem(h,d,to);i[to]=true;j[to]=true}}if(g=="down")for(d=
f.length-1;d>=0;--d){j[d]=false;if(f[d].selected){to=d+1;if(j[to])to=d;h=moveItem(h,d,to);i[to]=true;j[to]=true}}if(g=="bottom"){k=0;for(d=f.length-1;d>=0;--d)if(f[d].selected){to=sourcelength-k-1;if(to>f.length)to=f.length;h=moveItem(h,d,to);i[to]=true;k++}}for(d=0;d<f.length;++d){f[d]=new Option(h[d].title,h[d].value);f[d].selected=i[d]}writeREX(a,b,e)};writeREX=function(a,b,e){e=c("#"+e+a+" option");a=c("#"+b+a);b=[];for(var g=0;g<e.length;++g)b.push(e[g].text);a.val(b.join(","))};moveItem=function(a,
b,e){if(b==e||e<0)return a;tmp=a[b];if(b>e)for(index=b;index>e;index--)a[index]=a[index-1];else for(index=b;index<e;index++)a[index]=a[index+1];a[e]=tmp;return a};checkInput=function(a){c("#"+a).attr("checked","checked")};rexShowMediaPreview=function(){var a;a=c(this).hasClass("rex-widget-media")?c("input[type=text]",this).val():c("select option:selected",this).text();var b=c(".rex-media-preview",this),e="../index.php?rex_resize=246a__"+a;if(a&&a.length!=0&&c.inArray(extension,imageExtensions)){a=
c("img",b);if(a.length==0){b.html("<img />");a=c("img",b)}a.attr("src",e);b.css("height")=="auto"&&b.fadeIn("normal")}else b.slideUp("fast")};sly_disableLogin=function(a){var b=parseInt(a.html(),10)-1;a.html(b+"");if(b>0)setTimeout(sly_disableLogin,1E3,a);else{c("div.rex-message p span").html(c("#login_message").text());c("#loginformular input:not(:hidden)").attr("disabled","");c("#rex-form-login").focus()}};sly_startLoginTimer=function(){var a=c("div.rex-message p span strong");if(a.length==1){c("#loginformular input:not(:hidden)").attr("disabled",
"disabled");setTimeout(sly_disableLogin,1E3,a)}};sly_catsChecked=function(){var a=c("#allcats").is(":checked"),b=c("#allmcats").is(":checked");c("#userperm-cat").attr("disabled",a?"disabled":"");c("#userperm-media").attr("disabled",b?"disabled":"");a&&b?c("#cats_mcats_perms").slideUp("slow"):c("#cats_mcats_perms").slideDown("slow")}})(jQuery);
